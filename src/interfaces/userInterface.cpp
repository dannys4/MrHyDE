...
Teuchos::RCP<Teuchos::ParameterList> UserInterfaceFactory::UserInterface(const std::string &filename)
{
  ...
  else
  {
    ...
    if (!have_aux_phys)
    { // this is optional (unless Aux Discretization is defined)
    ...
    if (!have_aux_disc)
    { // this is optional (unless Aux Physics is defined)
